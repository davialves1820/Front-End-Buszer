...
export class FleetService {
  static async getFleetStats(): Promise<FleetStats> {
    ...
    const activeVehicles = mockVehicles.filter(v => v.status === 'active' || v.status === 'delayed' || v.status === 'early').length;
    const inactiveVehicles = mockVehicles.filter(v => v.status === 'inactive' || v.status === 'maintenance').length;
